...
class WidgetExtensionContractTests(unittest.TestCase):
    ...
    def test_tap_has_short_visible_feedback(self):
        ...
        self.assertIn("struct FlipGlyph: Identifiable", self.source)
        self.assertIn("struct FlipDigitText", self.source)
        self.assertIn("ForEach(glyphs)", self.source)
        self.assertIn("character.wholeNumberValue != nil", self.source)
        self.assertIn("private var digitDelay: Double", self.source)
        self.assertIn("private var digitDuration: Double", self.source)
        self.assertIn("rotation3DEffect", self.source)
        self.assertIn(".transition(.asymmetric(insertion: .push(from: insertionEdge), removal: .opacity))", self.source)
        self.assertIn(".animation(.easeInOut(duration: digitDuration).delay(digitDelay), value: refreshAnimationID)", self.source)
